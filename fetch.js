...
async function logos() {
  ...
  for (var [isin, x] of isins) {
    if (fs.existsSync(`logos/${isin}.log`)) continue;
    if (fs.existsSync(`logos/${isin}.png`)) continue;
    console.log(`Fetching ${isin} (${x.symbol}; ${x.description}) ...`);
    try {
      await download(`https://assets-netstorage.groww.in/stock-assets/logos/${isin}.png`, 'logos');
      console.log(`Fetched ${isin}. Waiting 10s ...`);
    }
    catch (e) {
      fs.writeFileSync(`logos/${isin}.log`, `${e.message}\n${x.symbol}; ${x.description}\n`);
